...
class KeyboardLayoutCalculator:
    ...
    def export_svg(self, filename: str = "keyboard_layout.svg"):
        ...
        # Calculate SVG dimensions (accounting for center coordinates)
        all_positions = positions['left'] + positions['right']
        min_x = min(x - self.footprint_width/2 for _, x, y in all_positions) - 10
        max_x = max(x + self.footprint_width/2 for _, x, y in all_positions) + 10
        min_y = min(y - self.footprint_height/2 for _, x, y in all_positions) - 25  # More space at top
        max_y = max(y + self.footprint_height/2 for _, x, y in all_positions) + 35  # More space at bottom
