...
class KeyboardLayoutConfig:
    ...
    def __init__(self, origin_x: float, origin_y: float, num_rows: int, column_stagger: Dict[int, float], num_thumb_keys: int, thumb_arc_col_start: int, thumb_arc_config: ThumbArcConfig, specific_positions: List[SpecificPosition]):
        """
...
class KeyboardLayoutCalculator:
    ...
    def calculate_diode_position(config: KeyboardLayoutConfig, switch_x: float, switch_y: float, switch_rotation: float = 0.0) -> Tuple[float, float, float]:
        ...
        if abs(switch_rotation) < 0.1:  # No rotation for main keys
            ...
            rotated_offset_x = config.diode_offset_x * math.cos(rot_rad) - config.diode_offset_y * math.sin(rot_rad)
            rotated_offset_y = config.diode_offset_x * math.sin(rot_rad) + config.diode_offset_y * math.cos(rot_rad)
    ...
    def generate_all_positions(config: KeyboardLayoutConfig) -> Dict[str, Dict[str, List[Tuple[str, float, float, float]]]]:
        ...
        """
        positions = {"left": {"switches": [], "diodes": []}, "right": {"switches": [], "diodes": []}}
...
def get_charma_config() -> KeyboardLayoutConfig:
    ...
    specific_positions = [
        ...
        SpecificPosition("PSW",  1,   3.75,  26.25,   90.0),
    ]
